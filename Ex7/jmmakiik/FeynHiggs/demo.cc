...
static void setPara(float mass) {
...
//  cout << "Width is " << GammaSMTot(3) << endl;
  return GammaSMTot(3);
...
int main() {
  ...
  float Width_value0 = Width();
  widths[0]=Width_value0;
  ofstream MyFile("output.txt");
  MyFile << 125.0 << " " << widths[0] << "\n";

  for(int i=1;i<N;i++) {
  	float step = (550.0-125.0)/100.0;
  	float m = static_cast<float>(125.0) + static_cast<float>(i)*static_cast<float>(step);
  	setPara(m);
  	...
  	float Width_value = Width();
  	
  	masses[i]=m;
  	widths[i]=Width_value;
  }

  for (int i = 1; i<N;i++) {
  	MyFile << masses[i] << " " << widths[i] << "\n";
  }
  MyFile.close();
}
